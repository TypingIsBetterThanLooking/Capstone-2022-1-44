...
static int dmz_reclaim_buf(struct dmz_reclaim *zrc, struct dm_zone *dzone)
{
	...
	if (ret == 0) {
		...
		printk("IN dmz_reclaim_buf, unmap zone is bzone, bzone id: %d, bzone cgroup: %d\n", bzone->id, bzone->cg->kn->id);
...
static int dmz_reclaim_seq_data(struct dmz_reclaim *zrc, struct dm_zone *dzone)
{
	...
	if(idx == CHUNK_PER_ZONE){
		printk("In dmz_reclaim_seq_data, idx is 4\n");
	...
	if (ret == 0) {
		 ...
		 * the buffer zone.
		 */
		printk("IN dmz_reclaim_seq_data, unmap zone is bzone, bzone id: %d, bzone cgroup: %d\n", bzone->id, bzone->cg->kn->id);
...
static int dmz_reclaim_rnd_data(struct dmz_reclaim *zrc, struct dm_zone *dzone)
{
	...
	kn_id = dzone->cg->kn->id;
 	...
 	unsigned int idx;
        if(dzone->cg == NULL){
		printk("IN dmz_reclaim_rnd_data, cg is NULL\n");	
                idx = dmz_chunk_cg(zmd, chunk, CHUNK_CG_UNMAPPED);
        }
        else{   
                idx = dmz_chunk_cg(zmd, chunk, dzone->cg->kn->id);
        ...
        if(idx == CHUNK_PER_ZONE){
                //idx = dmz_chunk_cg(zmd, chunk, CHUNK_CG_UNMAPPED);
		printk("In dmz_reclaim_rnd_data, no matching cg_id: %d chunk: %d, return idx: %d\n", kn_id, chunk, idx);
        }
