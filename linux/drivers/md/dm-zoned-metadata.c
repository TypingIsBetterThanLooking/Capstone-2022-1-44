...
#define CHUNK_PER_ZONE	4
#define CHUNK_CG_UNMAPPED	UINT_MAX
...
struct dmz_metadata {
	...
	/*Zone Cgroup information*/
	struct cgroup_zones     cz[10000];

	unsigned int		chunk_cg_zone[128][16];
};
...
static int dmz_load_mapping(struct dmz_metadata *zmd)
{
	...
	while (chunk < zmd->nr_chunks) {
		...
		if (dzone_id == DMZ_MAP_UNMAPPED){
			printk("IN dmz_load_mapping, dmz_chunk_cg chunk: %d index: %d\n", e, idx);
			goto next;
		}
		...
		/* Check buffer zone */
		bzone_id = le32_to_cpu(dmap[e].bzone_id[idx]);
...
static void dmz_set_chunk_mapping(struct dmz_metadata *zmd, unsigned int chunk,
				  unsigned int dzone_id, unsigned int bzone_id, unsigned int idx)
{
 ...
 */
struct dm_zone *dmz_get_chunk_mapping(struct dmz_metadata *zmd, unsigned int chunk, int op, struct bio *bio)
{
	struct dmz_mblock *dmap_mblk = zmd->map_mblk[chunk >> DMZ_MAP_ENTRIES_SHIFT];
	struct dmz_map *dmap = (struct dmz_map *) dmap_mblk->data;
	...
	//
	unsigned int kn_id = bio->bi_blkg->blkcg->css.cgroup->kn->id;
	unsigned int idx;
	unsigned int a;
	...
	dmz_lock_map(zmd);
again:
	// chunk에 해당하는 cg_zone에서 cgroup이 일치하는 인덱스를 찾아옴
	idx = dmz_chunk_cg(zmd, chunk, kn_id);
	//printk("bio oper: %d\n", op);

	// cgroup과 일치하는 인덱스가 없으면, cgroup이 할당되지 않은 인덱스 가져오기
	if(idx == CHUNK_PER_ZONE){
		idx = dmz_chunk_cg(zmd, chunk, CHUNK_CG_UNMAPPED);
		/*if(op == REQ_OP_WRITE){
			zmd->chunk_cg_zone[chunk][idx] = kn_id;
			printk("IN get_chunk_mapping, chunk: %d, cgroup_id: %d, cgroup: %s idx: %d\n", chunk, kn_id, bio->bi_blkg->blkcg->css.cgroup->kn->name, idx);
		}*/
	}

	if(idx == CHUNK_PER_ZONE){
		printk("ERROR: IN dmz_get_chunk_mapping, dmz_chunk_cg index is 4\n");
	}
...
//			printk("IN get_chunk_maping, goto again... chunk: %d, cgroup_id : %d, name: %s\n", chunk, kn_id, bio->bi_blkg->blkcg->css.cgroup->kn->name);
			goto again;
		}
		printk("IN get_chunk_mapping, new dzone alloc, chunk: %d, kn_id:%d\n", chunk, kn_id);
		// dzone에 cgroup 할당
	//	dzone->cg = bio->bi_blkg->blkcg->css.cgroup;
		tmp_flag = 1;
	...
	if(tmp_flag == 1)
		for(a=0;a<CHUNK_PER_ZONE;a++)
			printk("chunk[%d][%d] = %d\n", chunk, a, zmd->chunk_cg_zone[chunk][a]);
	tmp_flag = 0;
out:
	dmz_unlock_map(zmd);
...
void dmz_put_chunk_mapping(struct dmz_metadata *zmd, struct dm_zone *dzone)
{
	...
	if (bzone) {
		if (dmz_weight(bzone))
			...
			printk("IN dmz_put_chunk_mapping, bzone unmap\n");
	...
	if (dmz_is_active(dzone) || bzone || dmz_weight(dzone))
		...
		printk("IN dmz_put_chunk_mapping, dzone unmap\n");
...
again:
	...
	printk("dmz_get_chunk_buffer call !\n");
	unsigned int idx;
	if(dzone->cg == NULL){
                printk("In dmz_get_chunk buffer, zone cgroup is null\n");
                idx = dmz_chunk_cg(zmd, dzone->chunk, CHUNK_CG_UNMAPPED);
	}
        else{    
                idx = dmz_chunk_cg(zmd, dzone->chunk, dzone->cg->kn->id);
		bzone->cg = dzone->cg;
	}

	if(idx == CHUNK_PER_ZONE){
		printk("IN dmz_get_chunk_buffer ERROR with dmz_chunk_cg\n");
	}

	/* Update the chunk mapping */
	dmz_set_chunk_mapping(zmd, dzone->chunk, dzone->id, bzone->id, idx);
...
{	
	...
	if(idx == CHUNK_PER_ZONE)
		printk("IN dmz_map_zone, ERROR index is 4\n");
...
void dmz_unmap_zone(struct dmz_metadata *zmd, struct dm_zone *zone)
{
	...
	printk("dmz_unmap_zone call !, chunk: %d\n", chunk);
	//
	unsigned int idx;
	/*if(zone->cg == NULL){
                printk("zone cgroup is null\n");
                idx = dmz_chunk_cg(zmd, chunk, CHUNK_CG_UNMAPPED);
	}*/
	...
	if(idx == CHUNK_PER_ZONE){
		printk("IN dmz_unmap_zone, ERROR with dmz_chunk_cg chunk: %d\n", chunk);
		idx = dmz_chunk_cg(zmd, chunk, CHUNK_CG_UNMAPPED);
		for(a=0;a<CHUNK_PER_ZONE;a++)
			printk("chunk[%d][%d] = %d\n", chunk, a, zmd->chunk_cg_zone[chunk][a]);
	}
	...
	if (test_and_clear_bit(DMZ_BUF, &zone->flags)) {
		 ...
		 */
		printk("IN unmap_zone CASE 1, chunk: %d buffer zone %d clear\n", chunk, zone->id);
		 ...
		 * not be buffered.
		 */
		printk("In unmap_zone CASE 2, chunk: %d dzone %d is unmapped\n", chunk, zone->id);
	...
	dmz_set_chunk_mapping(zmd, chunk, dzone_id, DMZ_MAP_UNMAPPED, idx);

	zone->chunk = DMZ_MAP_UNMAPPED;
	
	printk("IN unmap_zone chunk[%d] for loop: \n", chunk);  
	for(a=0;a<CHUNK_PER_ZONE;a++){
		printk("chunk[%d][%d] = %d\n", chunk, a, zmd->chunk_cg_zone[chunk][a]);
	}
	list_del_init(&zone->link);
		     ...
		     const char *devname)
{
	...
	// Initialize Cgroup_zones array
        for (j = 0; j < 10000; j++){
                INIT_LIST_HEAD(&(zmd->cz[j].list));
        }
...
/* Add Log when zone is mapping 
void dmz_map_log(struct dm_zone *zone)
{
	FILE *log_fp;
	char message[1024];
	...
	do_gettimeofday64(&my_time);
        time_to_tm(my_time.tv_sec, 0, &my_date);

	strftime(message, 255, "\n[%H:%M:%S] :", &my_date);
	
	log_fp = fopen("/home/femu/mapping.log", "a");
	if(log_fp!=NULL){
		fprint(log_fp, message);
		fclose(log_fp);
		log_fp = NULL;
	}
}
...
/* Add zone_id in cz Array*/
void dmz_zone_cgroup_map(struct dmz_metadata *zmd, struct dm_zone *zone)
{
	struct cgroup_zones *temp = NULL;
	temp = (struct cgroup_zones *)kmalloc(sizeof(struct cgroup_zones), GFP_KERNEL);
	temp->zone_id = zone->id;
	list_add(&temp->list, &zmd->cz[zone->cg->kn->id].list);  
}

/* print cz[cgroup_id] entry */
void dmz_zone_cgroup_list_print(struct dmz_metadata *zmd, unsigned long long int cgroup_id)
{
	struct cgroup_zones *temp = NULL;
	struct list_head *pos = NULL;
	unsigned int i = 0;
	list_for_each(pos, &zmd->cz[cgroup_id].list){
		temp = list_entry(pos, struct cgroup_zones, list);
		//printk("cz[%llu]'s pos[%d] => zone_id=%d\n", cgroup_id, i, temp->zone_id);
		i++;
		if(i>10) break;
	}
}

/* check already zone_id in cz[cgroup_id]*/
bool dmz_zone_cgroup_check(struct dmz_metadata *zmd, unsigned long long int cgroup_id, unsigned int zone_id)
{
	struct cgroup_zones *temp = NULL;
        struct list_head *pos = NULL;
        unsigned int i = 0;
        list_for_each(pos, &zmd->cz[cgroup_id].list){
                temp = list_entry(pos, struct cgroup_zones, list);
		if(temp->zone_id == zone_id) return true;                
                i++;
        }
	return false;
}

unsigned int dmz_dev_zone_id_public(struct dmz_metadata *zmd, struct dm_zone *zone)
{
	return dmz_dev_zone_id(zmd, zone);
}


/*chunk's cgroup dzone index */
unsigned int dmz_chunk_cg(struct dmz_metadata *zmd, unsigned int chunk, unsigned int kn_id){
	
	unsigned int i;
	for(i=0; i<CHUNK_PER_ZONE; i++)
		if(zmd->chunk_cg_zone[chunk][i] == kn_id)
			break;
	//printk("IN dmz_chunk_cg, chunk: %d, kn_id: %d, idx: %d\n", chunk, kn_id, i);
	return i;

}

void dmz_set_szone(struct dmz_metadata *zmd, unsigned int idx, unsigned int chunk, unsigned int kn_id){
	zmd->chunk_cg_zone[chunk][idx] = kn_id;
}
